...
const mockJwtService = {
  sign: jest.fn(() => 'signed-token-baaby'),
  verify: jest.fn(),
};
...
describe('UsserService', () => {
  ...
  describe('login', () => {
    ...
    it('should return token if password correct', async () => {
      ...
      usersRepository.findOne.mockResolvedValue(mockedUser);
      const result = await service.login(loginArgs);
      expect(result).toEqual({ ok: true, token: 'signed-token-baaby' });
  ...
  describe('findById', () => {
    ...
    it('shuold fail if no user is found ', async () => {
  ...
  describe('verifyEmail', () => {
    it('shouold verify email', async () => {
      ...
      expect(verificationRepository.delete).toHaveBeenCalledWith(1);
      expect(result).toEqual({ ok: true });
    });
